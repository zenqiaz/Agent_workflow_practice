"""
Test: benzene + NO2+ EAS rigid-body approach scan.

Preloads benzene (fixed, q=0) and no2plus positioned above C0 at 2.5 Å (q=+1).
Plans with build_approach_scan_geometries + parallel run_sp_energy.

Usage:
  python test_eas_approach.py           # plan mode only
  TEST_MODE=full python test_eas_approach.py  # plan + execute
"""
...
import test_success_rate as tsr   # reuse _call_planner, _build_messages
...
# NO2+: N directly above C0 (0, 1.396, 0) at z=1.8 Å; O-N-O along X (parallel to ring)
...
MESSAGE = (
    "Compute the EAS bond-forming energy curve for benzene + NO2+. "
    "benzene (q=0) is the fixed host and no2plus (q=+1) is already positioned with N "
    "directly above C0 (atom 0 of benzene) at 1.8 Ang. "
    "Use build_approach_scan_geometries with mol_a_id=benzene, mol_b_id=no2plus, "
    "n_steps=9, step_ang=0.05, ref_atom_a=0, output_prefix=approach "
    "to scan C0-N from 1.8 Ang down to 1.4 Ang along the z-axis. "
    "Run monomer SPs for benzene and no2plus in parallel. "
    "Run SP on each of the 9 approach geometries (approach_00 through approach_08). "
    "Compute delta_E_int = E(dimer) - E(benzene) - E(no2plus) in kcal/mol at each step. "
    "Use B3LYP/def2-SVP."
)
...
def check_plan(plan: dict) -> dict:
    ...
    results = {
        ...
        "has_sp_approach":    tools.count("run_sp_energy") >= 5,  # ≥2 monomers + ≥3 approach
...
async def main():
    ...
    print(f"\n{'='*60}")
    print(f"EAS approach scan test  [mode={MODE}]")
    print(f"{'='*60}\n")
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Build state and preload geometries
            ...
            for n in plan.get("nodes", []):
                ...
                print(f"  [{kind}] {n.get('id','?'):30s}  {tool}")
