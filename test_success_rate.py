...
_CHECKER_DEFAULT_MESSAGES: dict = {
    ...
    "coordination_sp": (
        "Calculate properties of these coordination compounds using ORCA with "
        "B3LYP/def2-SVP in the gas phase. For each compound report: total energy (Hartrees), "
        "dipole moment (Debye), HOMO-LUMO gap (eV). "
        "Use build_coordination_complex to generate initial geometries, then optimize and run SP. "
        "Compounds: "
        "(1) hexacyanoferrate(III) [Fe(CN)6]^3-, Fe3+ low-spin d5, charge=-3, mult=2, "
        "ligands=['cn','cn','cn','cn','cn','cn'], geometry='oct'; "
        "(2) cisplatin cis-Pt(NH3)2Cl2, charge=0, mult=1, "
        "ligands=['nh3','nh3','cl','cl'], geometry='sqp'; "
        "(3) bis(ethylenediamine)copper(II) [Cu(en)2]^2+, charge=+2, mult=2, "
        "ligands=['en','en'], geometry='sqp' (en is bidentate, coord=4); "
        "(4) nickel acetylacetonate Ni(acac)2, charge=0, mult=1, "
        "ligands=['acac','acac'], geometry='sqp' (acac is bidentate, coord=4)."
    ),
}
...
def _all_product_keys(plan: dict) -> set:
    """Collect all artifact keys from product dicts and final_report.fields (auto-artifact mode)."""
    keys = set()
...
def check_sp(plan: dict, context: Optional[Dict[str, Any]] = None) -> Dict[str, bool]:
    nodes      = plan.get("nodes", [])
    tools_used = {n.get("tool") for n in nodes if n.get("kind") == "tool"}
    artifacts  = plan.get("artifacts_to_save") or []
    ...
    if context:
        mol_names  = context.get("molecule_names") or []
        expected_n = context.get("expected_n", 0)
        n_sp       = sum(1 for n in nodes if n.get("tool") == "run_sp_energy")
        n_energy   = sum(1 for k in all_keys if _is_energy(k))

        # Check 1: each molecule name has at least one artifact key containing it
        if mol_names:
            def _mol_in_keys(mol: str) -> bool:
                tokens = [t for t in re.split(r"[\s\-_]+", mol.lower()) if len(t) >= 3]
                return any(
                    any(tok in k.lower() for tok in tokens)
                    for k in all_keys
                )
            result["mol_names_in_artifacts"] = all(_mol_in_keys(m) for m in mol_names)

        # Check 2: node and artifact counts match expected N
        if expected_n > 0:
            result["sp_count_matches_n"]     = (n_sp    == expected_n)
            result["energy_count_matches_n"] = (n_energy == expected_n)
...
def check_coordination_sp(plan: dict) -> Dict[str, bool]:
    ...
    _EXPECTED = [
        ...
        (("ni", "dmg"),               0,     1),   # Ni(dmgH)2, Ni2+ d8 square planar S=0 (legacy)
        (("ni", "acac"),              0,     1),   # Ni(acac)2, Ni2+ d8 square planar S=0
    ...
    result = {
        ...
        "has_energy_art":      any(
            ("energy" in k and "_eh" in k) or
            (k.endswith("_eh") and (k.startswith("e_") or k.startswith("energy_")))
            for k in all_keys_l
        ),
...
def build_random_sp_message(csv_path: str, n: int, seed: Optional[int] = 0) -> Tuple[str, List[str]]:
    """Sample N compounds from a CSV and build an SP message for test_success_rate.

    Returns (message, names) so callers can pass names as checker_context.
    """
...
async def _run_full_suite(args):
    ...
    if getattr(args, "csv", None) and getattr(args, "n_compounds", None):
        message, mol_names = build_random_sp_message(args.csv, args.n_compounds,
                                                     seed=getattr(args, "seed", 0))
        checker_name    = "sp"
        checker_context = {"molecule_names": mol_names, "expected_n": args.n_compounds}
    else:
